...
func isUserAccountExist(c *gin.Context, userAccount string) bool {
	affected := global.DB.Take(&user, "user_account = ?", userAccount).RowsAffected
	if affected == 1 {
...
func getContextValue(ctx *gin.Context, key string) interface{} {
...
func bindContextJson(ctx *gin.Context, data interface{}) bool {
	err := ctx.ShouldBindJSON(&data)
	if err != nil {
		fmt.Println("[api user bindContextJson err] c.ShouldBindJSON : ", err.Error())
		ctx.JSON(http.StatusForbidden, utils.ResponseError(utils.ParamsError, "缺少必填参数或不合法！"))
		return false
	}
	return true
}
...
func updateUserInfo(ctx *gin.Context, user models.UserInfo) bool {
	//零值不更新
	affected := global.DB.Take(&user, user.ID).Updates(models.UserModel{
	...
	if affected == 0 {
		ctx.JSON(http.StatusInternalServerError, utils.ResponseError(utils.MysqlError, "Mysql修改用户信息错误！"))
		return false
	}
	return true
}

func changePassword(ctx *gin.Context, user models.UserChangePassword) bool {
	// 判断密码和确认密码是否一致
	if user.UserPassword != user.CheckPassword {
		ctx.JSON(http.StatusForbidden, utils.ResponseError(utils.ParamsError, "两次密码不一样！"))
		return false
	}
	// 密码加密
